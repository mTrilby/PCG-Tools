...
namespace Domain.Model.Common.Synth.Meta
{
    ...
    public abstract class Bank<T> : ObservableObject, IBank where T:IPatch
    {
        ...
        protected Bank(IBanks banks, BankTypeEType type, string id, int pcgId)
        {
            _banks = banks;
            Type = type;

            // A GM bank is always loaded.
            if (type == BankTypeEType.Gm)
            {
                IsLoaded = true;
            }
        ...
        public int CountFilledAndNonEmptyPatches
        {
            get
            {
                return Patches.Count(patch =>
                    ((IBank) (patch.Parent)).IsLoaded && !patch.IsEmptyOrInit && (Type != BankTypeEType.Gm));
            }
        }
