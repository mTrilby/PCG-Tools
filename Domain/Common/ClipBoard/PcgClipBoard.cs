...
namespace Domain.Common.ClipBoard
{
    ...
    public class PcgClipBoard : IPcgClipBoard
    {
        ...
        /// <summary>
        /// </summary>
        public List<IClipBoardPatches> Programs { get; } // List per Program.SynthesisType.Last
        ...
        public void CopySetListSlotToClipBoard(ISetListSlot setListSlot, bool clearAfterCopy)
        {
            ...
            if (setListSlot.PcgRoot.Content != null)
            {
                ...
                if (!CutPasteSelected)
                {
                    ...
                    if (usedPatch != null &&
                        (SettingsDefault.CopyPaste_CopyPatchesFromMasterFile || !usedPatch.IsFromMasterFile))
                    {
        ...
        public int CountUncopiedSampledPrograms()
        {
            ...
            for (var index = 0; index < (int)ProgramBank.SynthesisType.Last; index++)
            {
                if (!Synth.PatchPrograms.Program.IsModeled((ProgramBank.SynthesisType)index))
                {
        ...
        private static IClipBoardPatch FindProgram(IEnumerable<IClipBoardPatch> programs, IPatch programToFind)
        {
            ...
            foreach (var program in programs)
            {
                ...
                if (Util.ByteCompareEqual(
                        programToFind.PcgRoot.Content, programToFind.ByteOffset, program.Data,
                        programToFind.ByteLength))
                {
                    patch = program;
                    break;
                }
        ...
        private static IClipBoardDrumKit FindDrumKit(IEnumerable<IClipBoardPatch> drumKits, IPatch drumKitToFind)
        {
            ...
            foreach (var drumKit in drumKits)
            {
                ...
                if (Util.ByteCompareEqual(
                        drumKitToFind.PcgRoot.Content, drumKitToFind.ByteOffset, drumKit.Data,
                        drumKitToFind.ByteLength))
                {
                    patch = drumKit;
                    break;
                }
        ...
        private bool ShouldTimbreBeCopied(ITimbre timbre, IBank usedProgramBank, IProgram usedProgram)
        {
            ...
            if (copy)
            {
                ...
                copy &= usedProgramBank.IsLoaded ||
                        (SettingsDefault.CopyPaste_CopyPatchesFromMasterFile && usedProgram.IsFromMasterFile);
            }
        ...
        {
            var clipBoardDrumKitToAdd = FindDrumKit(drumKit) ??
        ...
        {
            var clipBoardDrumPatternToAdd = FindDrumPattern(drumPattern) ??
        ...
        private static void FixProgramReferencesToSetLists(IClipBoardPatch patch, IProgram program, IPcgMemory memory)
        {
            ...
            if (memory.SetLists != null)
            {
                foreach (var slot in memory.SetLists.BankCollection.Where(
                             ...
                             .SelectMany(list => list.Patches, (list, patch1) => (ISetListSlot)patch1).Where(slot =>
                                 slot.SelectedPatchType == SetListSlot.PatchType.Program &&
        ...
        private void FixPasteDrumKitReferencesInPrograms(IClipBoardPatch drumKit)
        {
            for (var index = 0; index < (int)ProgramBank.SynthesisType.Last; index++)
            {
                foreach (var clipBoardPatch in Programs[index].CopiedPatches)
                {
                    ...
                    for (var drumKitIndex = 0;
                         drumKitIndex < program.ReferencedDrumKits.CopiedPatches.Count;
                         drumKitIndex++)
                    {
