    ...
    public class MdiContainer : UserControl
    {
            ...
            switch (e.Action)
            {
                case NotifyCollectionChangedAction.Add:
                {
                    ...
                    if (_windowCanvas.Children.Count == 0 ||
                        ((MdiChild)_windowCanvas.Children[_windowCanvas.Children.Count - 1]).WindowState !=
        ...
        internal void InvalidateSize()
        {
            ...
            for (var i = 0; i < _windowCanvas.Children.Count; i++)
            {
                ...
                var farPosition = new Point(Canvas.GetLeft(mdiChild) + mdiChild.ActualWidth,
                    Canvas.GetTop(mdiChild) + mdiChild.ActualHeight);
        ...
        private static void ThemeValueChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e)
        {
            ...
            switch (themeType)
            {
                case ThemeType.Luna:
                    ...
                    Application.Current.Resources.MergedDictionaries.Add(_currentResourceDictionary =
                        (ResourceDictionary)XamlReader.Load(
                            Application.GetResourceStream(new Uri(@"WPF.MDI;component/Themes/Luna.xaml",
                ...
                case ThemeType.Aero:
                    ...
                    Application.Current.Resources.MergedDictionaries.Add(_currentResourceDictionary =
                        (ResourceDictionary)XamlReader.Load(
                            Application.GetResourceStream(new Uri(@"WPF.MDI;component/Themes/Aero.xaml",
                                UriKind.Relative)).Stream));
                    // ReSharper restore PossibleNullReferenceException
                    break;
            }
