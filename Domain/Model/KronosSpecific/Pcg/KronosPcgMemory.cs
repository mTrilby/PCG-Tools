...
namespace Domain.Model.KronosSpecific.Pcg
{
    ...
    public class KronosPcgMemory : KronosOasysPcgMemory
    {
        public KronosPcgMemory(string fileName)
            : base(fileName, ModelsEModelType.Kronos)
        {
        ...
        protected override void FixChecksumValues(PcgMemoryChecksumType checksumType)
        {
            ...
            var startIndex = checksumType == PcgMemoryChecksumType.Kronos1516 ? 3 : 0; // For OS1.5/1.6, skip DIV1 and INI2/3.
            for (var index = startIndex; index < Chunks.Collection.Count; index++)
            {
                ...
                if (checksumChunks.Contains(chunk.Name))
                {
                    ...
                    if (checksumType == PcgMemoryChecksumType.Kronos1516)
                    {
                        // Save in INI2.
        ...
        public override void CopyPatch(IClipBoardPatch patchToPaste, IPatch patch)
        {
            ...
            if (Model.OsVersion == ModelsEOsVersion.EOsVersionKronos15_16)
            {
                if (patch is KronosProgram)
                {
                    // Copy PRG2 content.
                    var programToPaste = (IClipBoardCombiKronosOs1516) patchToPaste;
                ...
                {
                    // Copy CBK2 content.
                    var combiToPaste = (IClipBoardCombiKronosOs1516) patchToPaste;
                ...
                else
                {
                    ...
                    if (slot != null)
                    {
                        Util.SetInt(this, Content, slot.Stl2BankOffset, 1,
                                    ((IClipboardSetListSlotKronosOs1516) patchToPaste).KronosOs1516Bank);
                        Util.SetInt(this, Content, slot.Stl2PatchOffset, 1,
                                    ((IClipboardSetListSlotKronosOs1516) patchToPaste).KronosOs1516Patch);
                    }
                }
        ...
        public override void CopyPatch(IPatch patchToPaste, IPatch patch)
        {
            ...
            if (Model.OsVersion == ModelsEOsVersion.EOsVersionKronos15_16)
            {
                ...
                {
                    // Copy PRG2 content.
                    var programToPaste = (IClipBoardProgramKronosOs1516)patchToPaste;
                    for (var parameter = 0; parameter < programToPaste.KronosOs1516Content.Length; parameter++)
                    {
