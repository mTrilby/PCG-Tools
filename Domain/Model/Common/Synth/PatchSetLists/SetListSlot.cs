...
namespace Domain.Model.Common.Synth.PatchSetLists
{
    ...
    public abstract class SetListSlot : Patch<SetListSlot>, ISetListSlot // , INavigable
    {
        ...
        /// 
        /// </summary>
        public abstract SetListSlotTextSize SelectedTextSize { get; set; }
        ...
        public bool IsCompleteInPcg
        {
            get
            {
                ...
                {
                    return (((IBank)(UsedPatch.Parent)).IsLoaded || (((ProgramBank)UsedPatch.Parent).Type != BankTypeEType.Gm));
                }
        ...
        public void ChangeVolume(ChangeVolumeParameters.ChangeVolumeParameters parameters, int minimumValue, int maximumValue)
        {
            switch (parameters.ChangeType)
            {
                case ChangeVolumeParametersEChangeType.Fixed:
                    Volume = parameters.Value;
                    break;

                case ChangeVolumeParametersEChangeType.Relative:
                    Volume = MathUtils.ClipValue(Volume + parameters.Value, 0, 127);
                    break;

                case ChangeVolumeParametersEChangeType.Percentage:
                    Volume = (int)(Volume * (float)parameters.Value / 100.0 + 0.5);
                    break;

                case ChangeVolumeParametersEChangeType.Mapped:
                    Volume = MathUtils.MapValue(Volume, 0, 127, parameters.Value, parameters.ToValue);
                    break;

                case ChangeVolumeParametersEChangeType.SmartMapped:
