...
namespace PcgTools.Model.KronosSpecific.Synth
{
    ...
    public class KronosCombiBank : KronosOasysCombiBank
    {
        ...
        {
            Debug.Assert(parameterIndex >= 0 && parameterIndex <
                KronosCombiBanks.ParametersInCbk2Chunk * KronosTimbres.TimbresPerCombiConstant);
            Debug.Assert(combiIndex >= 0 && combiIndex <= CountPatches);

            return Cbk2PcgOffset +
                   parameterIndex * CountPatches *
                   KronosTimbres.TimbresPerCombiConstant + // = 0 if parameterIndex = 0, else start of parameter 1
