...
namespace Domain.Model.Common.Synth.PatchPrograms
{
    ...
    public abstract class Program : Patch<Program>, IProgram // , INavigable
    {
        ...
        /// <returns></returns>
        public static bool IsModeled(ProgramBankSynthesisType type)
        {
...
// ReSharper disable once UnusedVariable
        ...
        public string SubCategoryAsName
        {
            get
            {
                ...
                if (((ProgramBank) Bank).Type == BankTypeEType.Gm)
                {
                    return "???";
                }
        ...
        private void ChangeReferencesFromSetLists(IPatch newPatch)
        {
            ...
            foreach (var setListSlot in
                ...
                where (setListSlot.SelectedPatchType == PatchType.Program) &&
